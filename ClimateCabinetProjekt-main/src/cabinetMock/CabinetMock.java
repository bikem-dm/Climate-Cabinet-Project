	...
	private boolean processMessage(String line, PrintWriter toClient) throws InterruptedException {
		...
		case PRETST: {
			...
			if (this.slots[slotNumber] != null) {
				...
				CabinetMock.log("SENDING - Slot[" + (slotNumber + 1) + "] NOK - No or faulty examinee in slot <<<"
						+ (slotNumber + 1) + ">>>");
				toClient.println("Slot[" + (slotNumber + 1) + "] NOK - No or faulty examinee in slot <<<"
						+ (slotNumber + 1) + ">>>");
			}
			this.preTestCnt++;
			this.awaited = MessageType.PRETST;
		...
		case STRTBURNIN: {
			...
			this.getActualTemp();
			CabinetMock
					.log("SENDING - Starting BURN-IN Test with actual room temperature of <<<" + this.actTemp + ">>>");
		...
		case OPERTEMP: {
			if (this.targetTempSet && !this.targetTempReached) {
				...
				if (this.actTemp >= 0) {
					if (this.actTemp >= (this.targetTemp - this.targetTemp * this.toleranceRate)
							&& this.actTemp <= (this.targetTemp + this.targetTemp * this.toleranceRate)) {
...
//		this.tempFailureRate = Float.parseFloat(tempFailureRate);
	...
	private String processStartMessage(StringTokenizer tokenizer) {
		// Expected message: "CabinetControl1|Armin Müllner|Admin|10|3";
